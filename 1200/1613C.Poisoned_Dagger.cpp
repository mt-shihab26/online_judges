...
vector<int> get_randoms(int a, int f, int l) {
    vector<int>t;srand(time(0));for(int i=0;i<a;i++)
    t.push_back(f+((rand()%(l-f+1))));return t; }
template <typename Tp> void print(Tp arr[], int n) {
    for(int i=0;i<n;i++)cout<<arr[i]<<" ";cout<<'\n';}
template <typename Tp> void print(const Tp &a) {
    for(const auto &ith:a)cout<<ith<<" ";cout<<'\n';}
template <class Tp,class Tp2>void print(umap <Tp, Tp2> a){
